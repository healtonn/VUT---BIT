	...
	<nav class="navbar navbar-static-top"> 
		<div class="container">
			...
			<ul class="nav navbar-nav">
			  ...
			  <li <?if($stranka==4){?>class="selected"<?}?>><a href="?menuSelector=4">Seznam správců</a></li>
			  <li <?if($stranka==5){?>class="selected"<?}?>><a href="?menuSelector=5">Seznam administrátorů</a></li>
			  <li <?if($stranka==6){?>class="selected"<?}?>><a href="?menuSelector=6">Přidat správce</a></li>
			  <li <?if($stranka==7){?>class="selected"<?}?>><a href="?menuSelector=7">Odhlásit</a></li>
			</ul>
...
?>
    ...
    <?
      break;
    case 4:       //seznam spravcu
      $dotaz = "SELECT * FROM Spravce";
      $odpoved=mysql_query($dotaz, $databaze);
    ?>
	<div class="panel panel-default seznamJizd">
		<div class="panel-heading">
			<h4 class="prihlas">Seznam správců</h4>
		</div>
		<div class="panel-body tabulka">
			<table class="table table-hover kartaZamestnanceTable">
			<?while($zamestnanec=mysql_fetch_array($odpoved, MYSQL_ASSOC)){?>
			  <tr>
				<div class="fotoZamestnance"><td rowspan="2">FOTO</td></div>
				<div class="idZamestnance">
				  <th>ID:</th>
				  <td><?echo $zamestnanec["idSpravce"]?></td>
				</div>
				<div class="jmenoZamestnance">
				  <th>Jmeno:</td>
				  <td><?echo $zamestnanec["jmeno"]?></td>
				</div>
				</div class="prijmeniZamestnance">
				  <th>Prijmeni:</th>
				  <td><?echo $zamestnanec["prijmeni"]?></td>
				</div>
			  </tr>
			  <tr>
				<div class="loginZamestnance">
				  <th>Login:</th>
				  <td><?echo $zamestnanec["login"]?></td>
				</div>
				<div class="hesloZamestnance">
				  <th>Heslo: </th>
				  <td><?echo $zamestnanec["heslo"]?></td>
				</div>
				<form method="POST">
					<td><input type="submit" name="upravaZaznamuSpravce" value="Upravit" class="btn btn-default"></td>
					<td><input type="submit" name="smazatZaznamSpravce" value="Smazat" class="btn btn-default"></td>
				</form>
			  </tr>
			<?} //konec while cyklu?>
			</table>
		</div>
    </div>    <!--konec div class seznam Spravce-->
    <?
      break;
    case 5:       //seznam administratoru
      $dotaz = "SELECT * FROM Admin";
      $odpoved=mysql_query($dotaz, $databaze);
    ?>
	<div class="panel panel-default seznamJizd">
		<div class="panel-heading">
			<h4 class="prihlas">Seznam administrátorů</h4>
		</div>
		<div class="panel-body tabulka">
			<table class="table table-hover kartaZamestnanceTable">
			<?while($zamestnanec=mysql_fetch_array($odpoved, MYSQL_ASSOC)){?>
			  <tr>
				<div class="fotoZamestnance">
					<td rowspan="2">FOTO</td>
				</div>
				<div class="idZamestnance">
				  <th>ID:</th>
				  <td><?echo $zamestnanec["idAdmina"]?></td>
				</div>
				<div class="jmenoZamestnance">
				  <th>Jmeno:</td>
				  <td><?echo $zamestnanec["jmeno"]?></td>
    ...
    <?
      break;
    case 6:       //pridat spavce
    ?>
	<div class="panel panel-default seznamJizd">
		<div class="panel-heading">
			<h4 class="prihlas">Přidat správce</h4>
		</div>
		<div class="panel-body tabulka">
		<div class="nove">
			<p>Vytváříte nový záznam o správci. Všecha pole musí být vyplněna!</p>
			<div class="form-inside">
			  <form method="POST" class="form-vertical">
				<div class="form-group">
					<label for="typZamestnance" class="control-label">Typ zaměstnance</label>
					<div class="radio">
						<label><input type="radio" name="typZamestnance" value="Administrator">Administrátor</label>
					</div>
					<div class="radio">
						<label><input type="radio" name="typZamestnance" value="Spravce">Správce</label>
					</div>
				</div>
				<div class="form-group">
					<label for="jmenoRidice" class="control-label">Jméno</label>
